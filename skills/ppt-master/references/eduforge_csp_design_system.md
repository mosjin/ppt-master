...
### 7. 单面板（full-width content panel）——三铁律（ppt-master#11）

整页只有一个贯穿宽度的内容面板（如 `x=120 y=180 w=1040`）时，**面板高度必须随内容收缩，禁止固定高 + 顶部堆字**。`svg_quality_checker.py` 的 `_check_single_panel_layout`（warning）会拦截下述三缺陷：

1. **空彩色 header 色条** —— 面板顶若加 act 色横条，**必须**含 section/帧标题文本；空色条禁止。需要彩条就把标题写进去；只想分隔就用细 `<line>`，不要实心 rect。
2. **首行 clip** —— 面板内首行 `<text>` 基线距面板上沿 **≥ 14px**（含 header 条则在条下沿 +padding），否则文字上沿被裁。
3. **面板下方大空白** —— 内容若顶部堆叠（首行距顶 < 60px）且底部留空 > 面板高 30%，即判定欠填充。修法二选一：**(a) 面板高度收缩贴合内容**（推荐，`h` = 内容实际高 + 上下 padding）；**(b) 内容垂直分布填满面板**（行距撑开或分块）。

```svg
<!-- ✅ 正确：面板高贴合内容（内容 5 行 → 面板 h≈210，非固定 320） -->
<rect x="120" y="180" width="1040" height="210" rx="10" fill="#EDE4D1"/>
<text x="160" y="222" font-size="14">第 1 行（距面板顶 42px，不裁）</text>
<!-- ... 末行 y≈360，面板底 y=390，底部留白 ≤30% ... -->

<!-- ❌ 错误：固定 h=320 但内容只到 y=382，下方 118px(37%) 空 -->
```
